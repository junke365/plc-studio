...
#define SL_VERSION_SUB   0x0F    /* 子版本掩码 */
#define SL_VERSION_MAIN  0xF0    /* 主版本掩码 */

/* 帧标志位（SL_FRAME_FLAG1 系列等）已在 smartlink.h 定义 */

/* ========== SDO 命令层头尺寸 ========== */

#define SL_SDO_CMDL_HDR_FIXED_SIZE           8   /* 固定头大小 */
#define SL_SDO_CMDL_HDR_VAR_SIZE             4   /* 可变头大小 */
#define SL_SDO_CMDL_HDR_WRITEBYINDEX_SIZE    4   /* WriteByIndex 头 (index+subindex+reserved) */
#define SL_SDO_CMDL_HDR_READBYINDEX_SIZE     4   /* ReadByIndex 头 */
#define SL_SDO_CMDL_HDR_WRITEMULTBYINDEX_SIZE 8  /* WriteMultByIndex 子头 */

/* ========== SDO 命令层标志 ========== */

#define SL_SDO_CMDL_FLAG_RESPONSE    0x80
#define SL_SDO_CMDL_FLAG_ABORT       0x40
#define SL_SDO_CMDL_FLAG_EXPEDITED   0x00
#define SL_SDO_CMDL_FLAG_SEGMINIT    0x10
#define SL_SDO_CMDL_FLAG_SEGMENTED   0x20
#define SL_SDO_CMDL_FLAG_SEGMCOMPL   0x30
#define SL_SDO_CMDL_FLAG_SEGM_MASK   0x30
#define SL_SDO_CMDL_FLAG_PADSIZE_MASK 0x03

/* ========== NMT 命令数据标志 ========== */

#define SL_NMT_CMD_DATA_FLAG_DELAY   0x01   /* NMT GoToStandby 含 MNSwitchOverDelay */

/* ========== 错误条目类型 ========== */

#define SL_ERR_ENTRYTYPE_STATUS       0x8000
#define SL_ERR_ENTRYTYPE_HISTORY      0x0000
#define SL_ERR_ENTRYTYPE_EMCY         0x4000
#define SL_ERR_ENTRYTYPE_MODE_ACTIVE  0x1000
#define SL_ERR_ENTRYTYPE_MODE_CLEARED 0x2000
#define SL_ERR_ENTRYTYPE_MODE_OCCURRED 0x3000
#define SL_ERR_ENTRYTYPE_MODE_MASK    0x3000
#define SL_ERR_ENTRYTYPE_PROF_VENDOR  0x0001
#define SL_ERR_ENTRYTYPE_PROF_SL     0x0002
#define SL_ERR_ENTRYTYPE_PROF_MASK    0x0FFF

/* ========== SyncControl / SyncStatus 位 ========== */

#define SL_SYNC_PRES_TIME_FIRST_VALID        0x00000001
#define SL_SYNC_PRES_TIME_SECOND_VALID       0x00000002
#define SL_SYNC_SYNC_MN_DELAY_FIRST_VALID    0x00000004
#define SL_SYNC_SYNC_MN_DELAY_SECOND_VALID   0x00000008
#define SL_SYNC_PRES_FALL_BACK_TIMEOUT_VALID 0x00000010
#define SL_SYNC_DEST_MAC_ADDRESS_VALID       0x00000020
#define SL_SYNC_PRES_MODE_RESET              0x40000000
#define SL_SYNC_PRES_MODE_SET                0x80000000
...
typedef struct
{
  ...
  SlNetTime   netTimeLe;       /* 可选：网络起始时间 (offset 20) */
  uint64_t     relativeTimeLe;  /* 可选：相对时间 us (offset 28) */
} SL_PACKED SlSocFrame;
...
typedef union
{
  SlSyncRequest syncRequest;    /* PollResponse Chaining 模式使用 (offset 24) */
} SlSoaPayload;
...
typedef struct
{
  ...
  uint8_t       slVersion;  /* MN 的 智能总线 版本 (offset 22) */
  uint8_t       flag3;             /* MN 冗余激活标志 (offset 23) */
  SlSoaPayload payload;           /* SoA 载荷 (offset 24) */
} SL_PACKED SlSoaFrame;
...
typedef struct
{
  ...
  SlNetTime timeStamp;     /* 时间戳 (offset 4) */
  uint8_t    aAddInfo[8];   /* 附加错误信息 (offset 12) */
} SL_PACKED SlErrHistoryEntry;
...
typedef struct
{
  ...
  SlErrHistoryEntry aErrorHistoryEntry[13]; /* 错误历史列表 (offset 32) */
} SL_PACKED SlStatusResponse;
...
typedef struct
{
  ...
  uint8_t  slProfileVersion;   /* CN 的协议版本 (offset 22) */
...
} SL_PACKED SlAsySdoComMultWriteReqReadResp;
...
} SL_PACKED SlAsySdoComWriteMultResp;
...
} SL_PACKED SlAsySdoComReadMultReq;
...
typedef struct
{
  ...
  SlAsySdoCom sdoSeqPayload;  /* SDO 载荷 (offset 4) */
} SL_PACKED SlAsySdoSeq;
...
typedef union
{
  SlStatusResponse     statusResponse;    /* StatusRequest 应答 */
  SlIdentResponse      identResponse;     /* IdentRequest 应答 */
  SlNmtCommandService  nmtCommandService; /* MN 的 NMT 命令 */
  SlNmtRequestService  nmtRequestService; /* CN 的 NMT 请求 */
  SlAsySdoSeq          sdoSequenceFrame;  /* SDO 传输 (ASnd 方式) */
  SlSyncResponse       syncResponse;      /* SyncRes */
...
typedef struct
{
  ...
  SlAsndPayload payload;    /* 服务数据 (offset 18) */
} SL_PACKED SlAsndFrame;
...
typedef union
{
  SlSocFrame  soc;    /* SoC (多播) */
  SlPreqFrame preq;   /* PReq (单播) */
  SlPresFrame pres;   /* PRes (多播) */
  SlSoaFrame  soa;    /* SoA (多播) */
  SlAsndFrame asnd;   /* ASnd (多播) */
} SlFrameData;
...
typedef struct
{
  ...
  uint8_t      messageType;   /* 消息类型 (SlMsgType 值, offset 14) */
  uint8_t      dstNodeId;     /* 目标节点 ID (offset 15) */
  uint8_t      srcNodeId;     /* 源节点 ID (offset 16) */
  SlFrameData data;          /* 帧数据 (offset 17) */
} SL_PACKED SlFrame;
