 ...
 *
 * 设备选择：环境变量 SL_EDRV_DEVICE 指定网卡名（如 eth0 / ens33），
 *           未指定时默认 "eth0"。
 */
...
#define SL_EDRV_DEVICE_MAX 64  /* 网卡名缓冲 */
#define SL_EDRV_RECV_TIMEOUT_MS 100 /* recvfrom 阻塞超时（用于退出检查） */
...
static SlEdrvLinux s_priv;

/* ========== 内部辅助 ========== */

/* 帧入队（满则丢弃最旧帧，保留最新数据） */
static void edrvQueuePush(SlEdrvLinux* d, const uint8_t* data, uint16_t len)
{
  SlEdrvRxSlot* slot;
...
/* 帧出队，返回是否成功 */
static bool edrvQueuePop(SlEdrvLinux* d, SlEdrvRxSlot* out)
{
  SlEdrvRxSlot* slot;
...
/* 按接收过滤器掩码匹配目的 MAC */
static bool edrvMatchFilter(const SlEdrvLinux* d, const uint8_t* frame, uint16_t len)
{
...
{
  SlEdrvLinux* d = (SlEdrvLinux*)arg;
  struct sockaddr_ll sll;
  socklen_t sllLen = sizeof(sll);
  uint8_t buf[SL_EDRV_MAX_FRAME];
  ssize_t n;
...
static int edrvLinuxInit(SlEdrv* self)
{
  SlEdrvLinux* d = (SlEdrvLinux*)self->priv;
  struct ifreq ifr;
  struct sockaddr_ll sll;
  struct timeval tv;
  const char* devName;
  ...
  tv.tv_sec = 0;
  tv.tv_usec = SL_EDRV_RECV_TIMEOUT_MS * 1000;
...
static int edrvLinuxExit(SlEdrv* self)
{
  SlEdrvLinux* d = (SlEdrvLinux*)self->priv;
...
static int edrvLinuxGetMacAddr(SlEdrv* self, uint8_t mac[6])
{
  SlEdrvLinux* d = (SlEdrvLinux*)self->priv;
...
static int edrvLinuxSend(SlEdrv* self, const uint8_t* frame, uint16_t len)
{
  SlEdrvLinux* d = (SlEdrvLinux*)self->priv;
  struct sockaddr_ll sll;
  ssize_t sent;
...
static int edrvLinuxSetRxFilter(SlEdrv* self, uint16_t filterMask)
{
  SlEdrvLinux* d = (SlEdrvLinux*)self->priv;
...
static int edrvLinuxSetRxCallback(SlEdrv* self, SlEdrvRxCallback cb, void* ctx)
{
  SlEdrvLinux* d = (SlEdrvLinux*)self->priv;
...
static int edrvLinuxLinkState(SlEdrv* self, SlLinkState* state)
{
  SlEdrvLinux* d = (SlEdrvLinux*)self->priv;
  struct ifreq ifr;
  int ret;
...
static int edrvLinuxPoll(SlEdrv* self, uint32_t timeoutMs)
{
  SlEdrvLinux* d = (SlEdrvLinux*)self->priv;
  SlEdrvRxSlot slot;
  struct timespec ts;
